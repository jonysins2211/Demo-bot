...
async def start_command(client: Client, message: Message):
    ...
    if len(text) > 7:
        ...
        for msg in messages:
            ...
            # --- Build reply markup (stream links generated on-demand via callback) ---
            existing_buttons = []
            if not DISABLE_CHANNEL_BUTTON and msg.reply_markup:
                try:
                    existing_buttons = msg.reply_markup.inline_keyboard
                except Exception:
                    pass
            reply_markup = InlineKeyboardMarkup(existing_buttons) if existing_buttons else None
            ...
            # --- STREAM_MODE: send on-demand stream/download button after each file ---
            if STREAM_MODE:
                try:
                    from FileStream.utils.file_properties import get_media_from_message, get_hash
                    media = get_media_from_message(msg)
                    if media:
                        file_id_str = f"{msg.id}"
                        await copied_msg.reply_text(
                            text="**•• ʏᴏᴜ ᴄᴀɴ ɢᴇɴᴇʀᴀᴛᴇ ᴏɴʟɪɴᴇ sᴛʀᴇᴀᴍ ʟɪɴᴋ ᴏғ ʏᴏᴜʀ ғɪʟᴇ ᴀɴᴅ ᴀʟsᴏ ғᴀsᴛ ᴅᴏᴡɴʟᴏᴀᴅ ʟɪɴᴋ ꜰᴏʀ ʏᴏᴜʀ ғɪʟᴇ ᴄʟɪᴄᴋɪɴɢ ᴏɴ ʙᴇʟᴏᴡ ʙᴜᴛᴛᴏɴ 👇**",
                            disable_web_page_preview=True,
                            reply_markup=InlineKeyboardMarkup([
                                [InlineKeyboardButton(
                                    "🚀 Fast Download / Watch Online 🖥️",
                                    callback_data=f"generate_stream_link:{file_id_str}"
                                )]
                            ])
                        )
                except Exception as e:
                    print(f"[STREAM_MODE] Could not send stream button: {e}")
